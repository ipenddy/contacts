...
@implementation ContactsPinyin



+ (NSMutableArray *)arrayWithPinYinFirstLetterFormat:(NSMutableArray *)items{
    if (![items count]) {
    ...
    for (ContactItem *item in items){
        NSString *firstLetter = item.firstchar;
    ...
    for (int i = 'A'; i <= 'Z'; i++) {
        ...
        NSMutableArray *array = dict[firstLetter];
        if ([array count]) {
            [array sortUsingComparator:^NSComparisonResult(ContactItem *obj1, ContactItem *obj2) {
